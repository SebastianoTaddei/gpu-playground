...
struct MPSDevice::Impl
{
  ...
  void cwise_op(Buffer const &a, Buffer const &b, Buffer &c, std::string const &kernel)
  {
    @autoreleasepool
    {
      ...
      auto const *mtl_a = static_cast<MPSBuffer const *>(a.get());
      auto const *mtl_b = static_cast<MPSBuffer const *>(b.get());
      auto *mtl_c       = static_cast<MPSBuffer *>(c.get());
...
MPSDevice::MPSDevice() : pimpl(std::make_unique<Impl>()) {}

MPSDevice::~MPSDevice() = default;

void MPSDevice::add(Buffer const &a, Buffer const &b, Buffer &c) const
{
  this->pimpl->cwise_op(a, b, c, "mat_add");
}

void MPSDevice::sub(Buffer const &a, Buffer const &b, Buffer &c) const
{
  this->pimpl->cwise_op(a, b, c, "mat_sub");
}

void MPSDevice::mul(Buffer const &a, Buffer const &b, Buffer &c) const
{
  @autoreleasepool
  {
    ...
    auto *descA = [MPSMatrixDescriptor matrixDescriptorWithRows:m
                                                        columns:k
                                                       rowBytes:k * sizeof(float)
                                                       dataType:MPSDataTypeFloat32];

    auto *descB = [MPSMatrixDescriptor matrixDescriptorWithRows:k
                                                        columns:n
    ...
    auto *matA = [[MPSMatrix alloc] initWithBuffer:mtl_a->buffer descriptor:descA];

    auto *matB = [[MPSMatrix alloc] initWithBuffer:mtl_b->buffer descriptor:descB];

    auto *matC = [[MPSMatrix alloc] initWithBuffer:mtl_c->buffer descriptor:descC];

    auto *mm = [[MPSMatrixMultiplication alloc] initWithDevice:this->pimpl->device
                                                 transposeLeft:NO
                                                transposeRight:NO
                                                    resultRows:m
                                                 resultColumns:n
...
void MPSDevice::cmul(Buffer const &a, Buffer const &b, Buffer &c) const
{
  this->pimpl->cwise_op(a, b, c, "mat_cmul");
}

void MPSDevice::cdiv(Buffer const &a, Buffer const &b, Buffer &c) const
{
  this->pimpl->cwise_op(a, b, c, "mat_cdiv");
}

void MPSDevice::sadd(Buffer const &a, Buffer const &b, Buffer &c) const
{
  this->pimpl->cwises_op(a, b, c, "mat_sadd");
}

void MPSDevice::ssub(Buffer const &a, Buffer const &b, Buffer &c) const
{
  this->pimpl->cwises_op(a, b, c, "mat_ssub");
}

void MPSDevice::smul(Buffer const &a, Buffer const &b, Buffer &c) const
{
  this->pimpl->cwises_op(a, b, c, "mat_smul");
}

void MPSDevice::sdiv(Buffer const &a, Buffer const &b, Buffer &c) const
{
  this->pimpl->cwises_op(a, b, c, "mat_sdiv");
}

Buffer MPSDevice::new_buffer(std::vector<float> data, Shape shape) const
{
  assert(this->pimpl->device != nil);

  MPSBuffer mtl_buffer{};
  ...
  return Buffer{
      HandlePtr{
          new MPSBuffer(mtl_buffer),
          [](void *ptr) -> void
          {
            auto *buf = static_cast<MPSBuffer *>(ptr);
...
void MPSDevice::copy_buffer(Buffer const &from, Buffer &to) const
{
  @autoreleasepool
  {
    assert_compatible_copy(from, to);

    auto const *mtl_from = static_cast<MPSBuffer const *>(from.get());
    auto *mtl_to         = static_cast<MPSBuffer *>(to.get());
...
void MPSDevice::transpose(Buffer const &from, Buffer &to) const
{
  @autoreleasepool
  {
...
std::vector<float> MPSDevice::cpu(Buffer const &buffer) const
{
  auto const *mtl_buf = static_cast<MPSBuffer const *>(buffer.get());
...
void MPSDevice::sync(Buffer const &buffer) const
{
  auto const *mtl_buf = static_cast<MPSBuffer const *>(buffer.get());
  cmd_wait_release(mtl_buf->last_cmd);
}
...
gpu_playground::DevicePtr gpu_playground::make_mps_device()
{
  return std::make_shared<gpu_playground::backend::MPSDevice>();
}
