...
void test_vec_mul(DevicePtr const &device, Tensor &a, Tensor &b)
{
  a.to(device);
  b.to(device);
...
int main()
{
  ...
  Shape const a_shape{.rows = 3, .cols = 1};
  Shape const b_shape{.rows = 1, .cols = 3};
  Tensor a(a_data, a_shape, serial_device);
  Tensor b(b_data, b_shape, serial_device);

  test_vec_mul(serial_device, a, b);
  test_vec_mul(eigen_device, a, b);
  test_vec_mul(simd_device, a, b);
  test_vec_mul(metal_device, a, b);
