...
void test_vec_sub(DevicePtr const &device, Tensor &a, Tensor &b)
{
  a.to(device);
  b.to(device);
...
int main()
{
  ...
  test_vec_sub(serial_device, a, b);
  test_vec_sub(eigen_device, a, b);
  test_vec_sub(simd_device, a, b);
  test_vec_sub(metal_device, a, b);
